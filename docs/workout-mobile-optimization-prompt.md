...
> 状态：**已被 2026-09 训练交互重构取代（superseded）**。本文描述的是旧版"逐组标记完成"交互，
> 当前实现已改为：无组级 `completed`，一组有数值即为训练记录；已录组收起、当前组展开；
> RPE/RIR/备注收进「更多」；训练时长由用户手动填写；上次每组明细；动作维度进展入口；组间歇只给按肌群（大/小）的建议文案，不倒计时也不写入记录。数据模型见 `docs/implementation-plan-v3.md` 7.2 节。
